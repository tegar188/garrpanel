...
try {
    ...
    echo "DB_ERROR: " . $e->getMessage(); exit;
}
...
if (!$row) { echo "NO_ROW"; exit; }
if ($row['expires_at'] && strtotime($row['expires_at']) < time()) { echo "EXPIRED"; exit; }
if ($row['hwid'] && $row['hwid'] !== $serial) { echo "HWID_MISMATCH"; exit; }
if (!$row['hwid']) {
